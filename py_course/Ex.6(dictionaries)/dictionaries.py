...
def solution10():
    ...
    sort_dict = sorted([(value, key) for (key,value) in things.items()], reverse=True)
    for tuple in sort_dict:
        left = N - sum
        if tuple[0] <= left:
            print(tuple[1],end=' ')
            sum += tuple[0]
...
def solution11():
    ...
    lst = [x.split(":") for x in lst_in]    # [['Пушкин', ' Сказака о рыбаке и рыбке'], ['Есенин', ' Письмо к женщине'], ['Тургенев', ' Муму'], ['Пушкин', ' Евгений Онегин'], ['Есенин', ' Русь']]
    


    # Solution 1. Requires lst var.
    d1 = {key: {i[1].strip() for i in lst if key == i[0]}     
            for key in {i[0]:None for i in lst}}          # create a dictionary and iterate through it's keys -->  {'Пушкин': None, 'Есенин': None, 'Тургенев': None}


    # Solution 2
    d2 = dict()
    [ d2.setdefault(key, set()).add(value) for key, value in (x.split(':') for x in lst_in) ]
    
    # Solution 3. Requires lst var.
    d3 = {i[0]: {j[1] for j in lst if i[0] == j[0]} for i in lst}
